...
// export default function FavoriteButton({ itemId, type = 'activity', className = '' }) {
...
//     try {
//       const res = await api.get(`/wishlist/check?itemId=${itemId}&type=${type}`);
//       setIsFavorite(res.data?.data?.inWishlist || false);
//     } catch (err) {
//       // Ignore errors
//     }
//   };
...
//       setIsFavorite(res.data?.data?.inWishlist);
//     } catch (err) {
...
//       disabled={loading}
//       className={`p-2 rounded-full bg-white/80 hover:bg-white transition-colors shadow-sm ${className}`}
