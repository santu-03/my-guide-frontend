...
// const MY_VIEW = (role) => ([
//   { to: "/dashboard/traveller", label: "Home", icon: Home },
//   ...(role === "traveller" ? [
//     { to: "/traveller/trips", label: "Trips", icon: MapPin },
//     { to: "/traveller/expenses", label: "Expenses", icon: CreditCard },
//     { to: "/traveller/reports", label: "Reports", icon: BarChart3 },
//     { to: "/traveller/cards", label: "Cards", icon: Layers },
//     { to: "/traveller/approvals", label: "Approvals", icon: ClipboardList },
//     { to: "/traveller/analytics", label: "Analytics", icon: BarChart3 },
//     { to: "/traveller/settings", label: "My Settings", icon: Settings },
//   ] : []),
// ]);

// const ADMIN_VIEW = [
//   { to: "/dashboard/admin", label: "Admin Home", icon: Home },
...
// export default function Sidebar({ role = "traveller", variant = "light" }) {
...
//       {!collapsed && <div className="text-xs uppercase tracking-wider text-gray-400 mb-2">{title}</div>}
//       {items.map(({ to, label, icon: Icon }) => {
//         const active = location.pathname === to || location.pathname.startsWith(`${to}/`);
//         return (
//           <Link key={to} to={to} className={linkCls(active)} onClick={() => setOpen(false)}>
...
//   return (
//     <>
//       <button onClick={() => setOpen(!open)} className="md:hidden fixed top-4 left-4 z-50 bg-primary-600 text-white p-2 rounded-lg shadow-md" aria-label="Toggle sidebar">
//         {open ? <X className="h-6 w-6" /> : <Menu className="h-6 w-6" />}
//       </button>

//       <aside
//         className={`fixed md:static top-0 left-0 h-full ${collapsed ? "w-20" : "w-64"} p-4 sm:p-6 flex flex-col justify-between transform transition-transform duration-300 z-40 ${bgClass} ${
//           open ? "translate-x-0" : "-translate-x-full md:translate-x-0"
//         }`}
//       >
//         <div className="space-y-6">
//           <Link to="/" className={`flex items-center gap-2 font-extrabold text-lg ${collapsed ? "justify-center" : ""}`} onClick={() => setOpen(false)}>
//             <div className="w-8 h-8 rounded-xl bg-primary-600 grid place-items-center text-white">TG</div>
//             {!collapsed && <span>Tour Guide</span>}
//           </Link>

//           {group("My View", MY_VIEW(role))}
//           {role === "traveller" && group("Shortcuts", [
//             { to: "/traveller/wishlist", label: "Wishlist", icon: Heart },
//             { to: "/traveller/calendar", label: "Calendar", icon: Calendar },
//           ])}
//           {group("Admin View", ADMIN_VIEW)}
//         </div>

//         <div className="space-y-2 border-t border-gray-200 dark:border-gray-800 pt-4">
//           <Link to="/settings" className={linkCls(false)}><Settings className="h-5 w-5" />{!collapsed && "Settings"}</Link>
//           <Link to="/logout" className={linkCls(false)}><LogOut className="h-5 w-5" />{!collapsed && "Logout"}</Link>
//           <button onClick={() => setCollapsed((v) => !v)} className="w-full text-sm mt-2 text-gray-500 hover:text-gray-900 dark:hover:text-white">
