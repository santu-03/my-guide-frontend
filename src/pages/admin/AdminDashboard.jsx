...
async function getBookings(params = {}) {
  ...
  return data?.bookings || data?.data?.bookings || data || [];
}
...
export default function AdminDashboard() {
  ...
  const fetchStats = async () => {
    try {
      ...
      const [usersRes, placesRes, activitiesRes, bookingsRes] = await Promise.all([
        getUsers({ limit: 1000 }),
      ...
      const places = Array.isArray(placesRes) ? placesRes : [];
      ...
      setStats({
        ...
        publishedActivities: activities.filter(a => a.isPublished).length,
  ...
  const fetchRecentActivities = async () => {
    try {
      const activities = await getActivities({ limit: 5, sortBy: 'createdAt', sort: '-createdAt' });
      setRecentActivities(Array.isArray(activities) ? activities.slice(0, 5) : []);
    } catch (err) {
      console.error('Error fetching recent activities:', err);
    }
  };
  ...
  const fetchTopPlaces = async () => {
    try {
      const [placesData, activitiesData] = await Promise.all([
        getPlaces({ limit: 100 }),
        getActivities({ limit: 1000 })
      ]);
      ...
      const placesWithCounts = places.map(place => {
        ...
        const activityCount = activities.filter(activity => {
          const activityPlaceId = activity.place?._id || activity.place?.id || activity.place || activity.placeId;
          return activityPlaceId === placeId;
        }).length;
        return { ...place, activityCount };
      });
  ...
  const fetchRecentUsers = async () => {
    try {
      const users = await getUsers({ limit: 4, sortBy: 'createdAt', sort: '-createdAt' });
  ...
  const metrics = useMemo(() => [
    ...
    { 
      ...
      label: "Featured Places", 
      value: stats.featuredPlaces?.toString() || "0",
      trend: { direction: "neutral", value: `${stats.totalPlaces || 0} total`, period: "" },
      color: "warning",
      to: "/admin/places"
    },
    { 
      ...
      label: "Total Revenue", 
      value: `₹${stats.totalRevenue?.toLocaleString() || '0'}`,
      trend: { direction: "up", value: `${stats.confirmedBookings} bookings`, period: "" },
      color: "success"
    },
  ], [stats]);
  ...
  return (
    <DashboardLayout role="admin" title="Admin Dashboard" user={user}>
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <p className="text-gray-600 dark:text-gray-400">Overview of your tour management system</p>
        </div>
        <Button 
          onClick={handleRefresh} 
          variant="outline"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm ring-1 ring-gray-200/70 dark:ring-gray-800/70 p-6">
          ...
          {loading ? (
            ...
            <div className="text-center py-8 text-gray-500">
              ...
              <Link to="/admin/activities/new" className="text-primary-600 hover:underline text-sm mt-2 inline-block">
            ...
            <div className="space-y-4">
              {recentActivities.map((activity) => (
                <div key={activity._id || activity.id} className="flex items-center gap-4 p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                  <div className="w-12 h-12 rounded-lg overflow-hidden flex-shrink-0">
                    <img 
                      src={activity.images?.[0] || '/placeholder-image.jpg'} 
                      alt={activity.title}
                      className="w-full h-full object-cover"
                      onError={(e) => e.target.src = '/placeholder-image.jpg'}
                    />
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm ring-1 ring-gray-200/70 dark:ring-gray-800/70 p-6">
          ...
          {loading ? (
            ...
            <div className="text-center py-8 text-gray-500">
              <p>No places found</p>
              <Link to="/admin/places/new" className="text-primary-600 hover:underline text-sm mt-2 inline-block">
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {recentUsers.map((userItem) => (
                <div key={userItem._id || userItem.id} className="flex items-center gap-3">
                  <img 
                    src={userItem.avatarUrl || '/default-avatar.png'} 
                    alt={userItem.name}
                    className="w-8 h-8 rounded-full object-cover"
                    onError={(e) => e.target.src = '/default-avatar.png'}
                  />
