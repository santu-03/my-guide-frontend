...
export default function BookingConfirm() {
  ...
  const handleDownloadReceipt = async () => {
    try {
      ...
      // Fallback: Print confirmation page
      toast.error("PDF download unavailable. Opening print dialog...");
      setTimeout(() => {
        window.print();
      }, 500);
    } finally {
  ...
  const handleShare = async () => {
    try {
      const shareData = {
        title: `Booking Confirmation - ${booking?.activity?.title}`,
        text: `I've booked ${booking?.activity?.title}!`,
        url: window.location.href,
      };
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-wrap gap-4 justify-center mb-8">
          <Button
            onClick={handleDownloadReceipt}
            variant="outline"
            startIcon={downloadingReceipt ? <Loader2 className="h-4 w-4 animate-spin" /> : <Download className="h-4 w-4" />}
