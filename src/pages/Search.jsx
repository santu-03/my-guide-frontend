...
//           ? api.get(`/places?${params.toString()}&limit=20`, { silenceToast: true }).catch(() => ({ data: { data: [] } }))
//           : Promise.resolve({ data: { data: [] } })
//       );
//       // activities
//       reqs.push(
//         (!searchFilters.type || searchFilters.type === 'activity')
//           ? api.get(`/activities?${params.toString()}&limit=20`, { silenceToast: true }).catch(() => ({ data: { data: [] } }))
//           : Promise.resolve({ data: { data: [] } })
//       );

//       const [placesRes, activitiesRes] = await Promise.all(reqs);
//       const places = Array.isArray(placesRes?.data?.data) ? placesRes.data.data : [];
...
//     const next = { ...appliedFilters, [key]: key === 'guests' ? '1' : '' };
//     setFilters(next);
...
//                 className={`p-2 ${viewMode === 'grid' ? 'bg-primary-600 text-white' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300'}`}
//               >
...
//                 className={`p-2 ${viewMode === 'list' ? 'bg-primary-600 text-white' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300'}`}
//               >
...
//             {/* Filter trigger */}
//             <Button variant="outline" onClick={() => setShowFilters(true)} className="relative lg:hidden">
...
//                 <button
//                   key={c.value}
//                   onClick={() => setFilters((p) => ({ ...p, category: active ? '' : c.value }))}
...
//               <Chip label={`Category: ${labelForCategory(appliedFilters.category)}`} onClear={() => removeFilter('category')} />
//             )}
//             {appliedFilters.priceRange && <Chip label={`Price: ${labelForPrice(appliedFilters.priceRange)}`} onClear={() => removeFilter('priceRange')} />}
//             {appliedFilters.rating && <Chip label={`${appliedFilters.rating}★ & up`} onClear={() => removeFilter('rating')} />}
//             {appliedFilters.date && <Chip label={new Date(appliedFilters.date).toLocaleDateString()} onClear={() => removeFilter('date')} />}
//             <button onClick={clearFilters} className="text-sm font-medium text-primary-700 dark:text-primary-400 hover:underline">
//               Clear all
//             </button>
//           </div>
//         )}
...
//                   <section>
//                     <SectionHeader
//                       title={`Destinations (${results.places.length})`}
//                       linkLabel={results.places.length > 4 ? 'View all destinations' : null}
//                       linkTo={
//                         results.places.length > 4
//                           ? `/search?${new URLSearchParams({ ...appliedFilters, type: 'place' }).toString()}`
//                           : undefined
//                       }
//                     />
//                     <div className={viewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6' : 'space-y-4'}>
//                       {results.places.map((p) => (
//                         <PlaceCard key={p._id} place={p} viewMode={viewMode} />
//                       ))}
//                     </div>
//                   </section>
//                 )}
...
//                   <section>
//                     <SectionHeader
//                       title={`Activities (${results.activities.length})`}
//                       linkLabel={results.activities.length > 8 ? 'View all activities' : null}
//                       linkTo={
//                         results.activities.length > 8
//                           ? `/search?${new URLSearchParams({ ...appliedFilters, type: 'activity' }).toString()}`
//                           : undefined
//                       }
//                     />
...
//           <div className="absolute right-0 top-0 bottom-0 w-full max-w-md bg-white dark:bg-gray-900 shadow-xl animate-in slide-in-from-right duration-200">
//             <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
//               <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Filters</h3>
//               <button onClick={() => setShowFilters(false)} className="p-2 rounded hover:bg-gray-100 dark:hover:bg-gray-800">
//                 <X className="h-5 w-5" />
//               </button>
//             </div>
//             <div className="p-4">
...
//     {linkLabel && linkTo && (
//       <Link to={linkTo} className="text-primary-600 hover:text-primary-700 font-medium text-sm flex items-center">
//         {linkLabel}
...
//   <span className="inline-flex items-center gap-1 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200 px-3 py-1 text-sm">
//     {label}
//     <button onClick={onClear} className="hover:text-red-600 dark:hover:text-red-400">
//       <X className="h-3.5 w-3.5" />
//     </button>
//   </span>
// );

// const FilterPanel = ({ filters, setFilters, activeFilterCount, applyFilters, clearFilters }) => (
...
// const FilterBody = ({ filters, setFilters }) => (
...
//     {/* Price */}
//     <div>
//       <label className="text-sm font-semibold text-gray-900 dark:text-white mb-3 block">Price Range</label>
//       <div className="space-y-2">
//         {PRICE_RANGES.map((r) => (
...
//     {/* Rating */}
//     <div>
//       <label className="text-sm font-semibold text-gray-900 dark:text-white mb-3 block">Minimum Rating</label>
//       <div className="space-y-2">
...
//   <Link to={`/places/${place._id}`} className={`group block ${viewMode === 'list' ? 'mb-4' : ''}`} aria-label={`Explore ${place.title}`}>
//     <Card
//       className={`overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-0.5 ${
//         viewMode === 'list' ? 'flex' : ''
//       }`}
//     >
//       <div
//         className={`relative bg-gradient-to-br from-primary-400 to-primary-600 overflow-hidden ${
//           viewMode === 'list' ? 'w-48 flex-shrink-0' : 'h-52'
//         }`}
//       >
//         <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent z-10" />
//         <div className="absolute inset-0 flex items-center justify-center">
//           <MapPin className="h-14 w-14 text-white/80" />
//         </div>

//         {place.city && (
//           <div className="absolute top-3 right-3 z-20">
//             <span className="bg-white/90 backdrop-blur text-gray-900 px-2 py-1 rounded-full text-xs font-medium">{place.city}</span>
//           </div>
//         )}
...
//         <h3 className="text-lg font-bold text-gray-900 mb-1 group-hover:text-primary-600 transition-colors dark:text-white line-clamp-2">
...
//               </div>
//             )}
//             {place.rating?.count && <span className="text-sm text-gray-500">({place.rating.count})</span>}
//           </div>
...
//   <Link to={`/activities/${activity._id}`} className={`group block ${viewMode === 'list' ? 'mb-4' : ''}`} aria-label={`Book ${activity.title}`}>
//     <Card
//       className={`overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-0.5 ${
//         viewMode === 'list' ? 'flex' : ''
//       }`}
//     >
//       <div
//         className={`relative bg-gradient-to-br from-secondary-400 to-secondary-600 overflow-hidden ${
//           viewMode === 'list' ? 'w-40 flex-shrink-0' : 'h-44'
//         }`}
//       >
//         <div className="absolute inset-0 flex items-center justify-center">
...
//             <span className="text-xs font-medium text-gray-800">{formatDuration(activity.durationMinutes)}</span>
//           </div>
//         </div>
//       </div>

//       <CardContent className={`${viewMode === 'list' ? 'flex-1' : ''} p-4`}>
//         <h3 className="font-bold text-gray-900 mb-1 line-clamp-2 group-hover:text-primary-600 transition-colors dark:text-white text-sm">
...
//                 {activity.rating.count && <span className="text-xs text-gray-500">({activity.rating.count})</span>}
//               </>
//             )}
//           </div>
//           <div className="text-right">
//             <div className="text-sm font-bold text-gray-900 dark:text-white">{formatPrice(activity.basePrice)}</div>
...
//       {hasActiveFilters ? 'Try adjusting your filters or search terms to find more options.' : 'Try a different search term or browse our popular destinations.'}
//     </p>
...
// const labelForCategory = (val) => (CATEGORIES.find((c) => c.value === val)?.label || val);
// const labelForPrice = (val) => (PRICE_RANGES.find((p) => p.value === val)?.label || val);
