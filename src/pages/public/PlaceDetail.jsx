...
export default function PlaceDetail(){
  ...
  useEffect(() => {
    api.get(`/places/${id}`).then(r => setData(r.data.data));
  }, [id]);
        ...
        return (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {data.rating?.avg && (
                <div>
                  <strong>Rating:</strong> {data.rating.avg}/5 ({data.rating.count} reviews)
                </div>
              )}
              {data.established && (
                <div>
                  <strong>Established:</strong> {data.established}
                </div>
              )}
          ...
          <div className="space-y-4">
            ...
            {data.location && (
              <div className="space-y-2">
                ...
                {data.location.city && <p><strong>City:</strong> {data.location.city}</p>}
                {data.location.country && <p><strong>Country:</strong> {data.location.country}</p>}
                {data.location.zipCode && <p><strong>Zip Code:</strong> {data.location.zipCode}</p>}
              </div>
            )}
          </div>
        );
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="border-b border-gray-200">
        <nav role="tablist" className="flex space-x-8 overflow-x-auto">
          {tabs.map((tab) => {
            const Icon = tab.icon;
            const isActive = activeTab === tab.id;
            return (
      ...
      <Button 
        to={`/search?q=${encodeURIComponent(data.location?.city || '')}`} 
        as="a"
      >
        See more in {data.location?.city || 'this city'}
      </Button>
    </div>
  );
}