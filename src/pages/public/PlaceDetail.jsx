  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="flex gap-2">
        {data.tags?.map(t=><span key={t} className="px-3 py-1 bg-gray-100 rounded-full text-sm">{t}</span>)}
      </div>
      <Button to={`/search?q=${encodeURIComponent(data.location?.city || '')}`} as="a">See more in {data.location?.city || 'this city'}</Button>
    </div>
  );
}
